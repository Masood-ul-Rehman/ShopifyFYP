    ...
    <header className="header px-4 md:px-6 w-full grad">
      <div className="w-100 max-w-7xl flex items-center justify-between mx-auto py-2">
        ...
        <div className="navbar">
          <ul className={(!openNav) ? 'hidden md:flex md:items-center md:relative md:py-0' : 'w-full md:w-auto flex flex-col md:flex-row absolute left-0 top-12 md:relative md:top-0 py-6 md:py-0 h-full items-center grad'}>
            {user ? (
              <li>
          ...
          <div className="mobile flex md:hidden items-center justify-end">
            {
              (!openNav) ? <MdToc size={40} color="white" onClick={()=>setOpenNav(!openNav)}/> 
              : <FaTimes size={40} color="white" onClick={()=>setOpenNav(!openNav)}/>
            }
          </div>
        </div>
      </div>
